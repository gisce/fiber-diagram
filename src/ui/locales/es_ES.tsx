export default {
  ok: "De acuerdo",
  cancel: "Cancelar",
  addSplitter: "Añadir splitter",
  addPatchPanel: "Añadir patch panel",
  inputs: "Entradas",
  outputs: "Salidas",
  selectInputsOutputs: "Selecciona el número de entradas y salidas",
  selectPorts: "Selecciona el número de puertos",
  ports: "Número de puertos simple",
  removeSplitter: "Eliminar splitter",
  removeSplitterConfirm:
    "Estás seguro que quieres eliminar el splitter? Se eliminaran todas las conexiones que tenga.",
  removeConnection: "Eliminar connexión",
  removeConnectionConfirm: "Estás seguro que quieres eliminar la conexión?",
  removePatchPanel: "Eliminar patch panel",
  removePatchPanelConfirm:
    "Estás seguro que quieres eliminar el patch panel? Se eliminaran todas las conexiones que tenga.",
};
